...
void zappy::engine::cmd::CmdBroadcast::cmdBroadcast(std::weak_ptr<entities::Player> player, World& world,
                                                    const std::string& args)
{
    ...
    {
        std::cout << debug::getTS() << "[TRACE] PLAYER " << sender->ID <<
            " SENT BROADCAST COMMAND WITH INCORRECT SYNTAX" << std::endl;
    ...
    EventSystem::trigger("player_broadcast", world.getGraphicalClients(), world.getMainZappyServer().getConfig(), world,
                         sender->ID, messageContent);
