...
void zappy::engine::GraphicalClient::sendGreetings(zappy::utils::ZappyConfig &config, const zappy::engine::World &world, const std::string& args) {
    zappy::engine::GraphicalClient::sendMsz(*this, config, world, args);
    zappy::engine::GraphicalClient::sendSgt(*this, config, world, args);
    zappy::engine::GraphicalClient::sendMct(*this, config, world, args);
    zappy::engine::GraphicalClient::sendTna(*this, config, world, args);
    const auto shareptr = this->shared_from_this();//std::shared_ptr<GraphicalClient>(this);
    const std::vector<std::shared_ptr<GraphicalClient>> vec = {shareptr};
    zappy::engine::GraphicalClient::sendPnw(vec, config, world, true);
    for (const auto& player: world.getPlayers()) {
        zappy::engine::GraphicalClient::sendPin(*this, config, world, std::to_string(player->ID));
        zappy::engine::GraphicalClient::sendPlv(*this, config, world, std::to_string(player->ID));
    }
    zappy::engine::GraphicalClient::sendEnw(*this, config, world);
}

//bad command
void zappy::engine::GraphicalClient::sendSuc(const unsigned int graphic_id, const zappy::engine::World &world) {
...
void zappy::engine::GraphicalClient::sendBct(GraphicalClient& graphic, zappy::utils::ZappyConfig &config, const World &world, const std::string& args) {
    ...
    for (auto [type, quantity] : world.getTileAt(x, y).getAllResources()) {
        switch (type) {
            case Ressources::FOOD: q0 = quantity; break;
            case Ressources::LINEMATE: q1 = quantity; break;
            case Ressources::DERAUMERE: q2 = quantity; break;
            case Ressources::SIBUR: q3 = quantity; break;
            case Ressources::MENDIANE: q4 = quantity; break;
            case Ressources::PHIRAS: q5 = quantity; break;
            case Ressources::THYSTAME: q6 = quantity; break;
        }
    }
...
void zappy::engine::GraphicalClient::sendPpoProxy(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int pl_id) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendPpo(GraphicalClient& graphic, zappy::utils::ZappyConfig &config, const World &world, const std::string& args) {
    ...
    com += std::to_string(n) + " " + std::to_string(player->getX()) + " " + std::to_string(player->getY()) + " " + std::to_string(static_cast<int>(player->getDirection()) + 1);
    world.getMainZappyServer().sendMessageToClient(com, graphic.getID());
}
...
void zappy::engine::GraphicalClient::sendPinProxy(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, unsigned int unused, const unsigned int pl_id) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendPnw(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, const zappy::utils::ZappyConfig &config, const World &world, const bool firsttime) {
    ...
    for (const auto& graphic : graphics) {
        for (const auto& p : players) {
            ...
            try {
                com += std::to_string(p->ID) + " " + std::to_string(p->getX()) + " " + std::to_string(p->getY()) + " " + std::to_string(static_cast<int>(p->getDirection()) + 1) + " " + std::to_string(p->getLevel()) + " " + config.teamNames[p->getTeamId()];
...
void zappy::engine::GraphicalClient::sendPdr(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int res_id, const unsigned int pl_id) {
    std::string com = "pdr ";
...
void zappy::engine::GraphicalClient::sendPex(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int pl_id) {
    for (const auto& graphic : graphics) {
        std::string com = "pex #";
...
void zappy::engine::GraphicalClient::sendPfk(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int pl_id) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendEbo(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int egg_id) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendEdi(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int egg_id) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendPdi(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int pl_id) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendSmg(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const std::string& message) {
    for (const auto& graphic: graphics) {
...
void zappy::engine::GraphicalClient::sendPgt(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const unsigned int res_id, const unsigned int pl_id) {
...
void zappy::engine::GraphicalClient::sendPic(const std::vector<std::shared_ptr<GraphicalClient>>& graphics, zappy::utils::ZappyConfig &config, const World &world, const std::vector<std::weak_ptr<engine::entities::Player>>& players) {
    ...
    for (const auto& graphic: graphics) {
        std::cout << debug::getTS() << "[TRACE][GRAPHIC] sending pic command to CLIENT : " << graphic->getID() << std::endl;
        world.getMainZappyServer().sendMessageToClient(com, graphic->getID());
    }
}
